...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  ...
  // Authorization: must be the assigned instructor AND still
  // actually assigned to this cohort (re-checked here, not just
  // trusted from creation time — directive §13).
  ...
  const stillAssigned = await instructorIsAssignedToCohort(instructor.instructorId, reqRow.cohort_id);
  if (!stillAssigned) {
    return NextResponse.json({ error: "You are no longer assigned to this cohort." }, { status: 403 });
  }
  ...
  const { data: updated, error } = await admin
    ...
    .eq("id", params.id)
    .eq("status", reqRow.status) // optimistic concurrency guard
    .select("*")
    .single();
