...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  ...
  if (!parsed.success) return NextResponse.json({ error: "reason is required to revoke a certificate" }, { status: 400 });
  ...
  if (fetchError || !certificate) return NextResponse.json({ error: "Certificate not found" }, { status: 404 });
  ...
  const { data: updated, error } = await client
    .from("certificates")
    .update({ status: "revoked", revoked_at: new Date().toISOString(), revoked_reason: parsed.data.reason })
    .eq("id", params.id)
    .eq("status", "issued")
    .select("*")
    .single();
